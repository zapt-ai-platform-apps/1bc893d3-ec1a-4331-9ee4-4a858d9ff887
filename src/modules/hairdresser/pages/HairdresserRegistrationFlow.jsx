...
import HairstyleSelection from './registration/HairstyleSelection';
...
export default function HairdresserRegistrationFlow() {
  ...
  const handlePersonalInfoSubmit = async (data) => {
    try {
      ...
      const response = await fetch('/api/users/register-hairdresser', {
        method: 'POST',
        headers: {
      ...
      // Move to hairstyle selection step
      setCurrentStep(2);
      navigate('/register/hairdresser/hairstyles');
    } catch (error) {
  ...
  const handleHairstyleSubmit = async (data) => {
    try {
      setFormData({ ...formData, ...data });
      
      // Save hairstyle selections to database
      const response = await fetch('/api/users/hairdresser-hairstyles', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${session?.access_token}`
        ...
        body: JSON.stringify({
          userId: user.id,
          hairstyles: data.selectedHairstyles,
          portfolioImages: data.portfolioImages
        })
      });
      
      if (!response.ok) {
        throw new Error('Failed to save hairstyle selections');
      }
      
      // Move to terms agreement step
      setCurrentStep(3);
      navigate('/register/hairdresser/terms');
    } catch (error) {
      console.error('Error saving hairstyle selections:', error);
      Sentry.captureException(error);
      alert('An error occurred while saving your hairstyle selections. Please try again.');
    }
  };
  ...
  const handleTermsSubmit = async (accepted) => {
    try {
      ...
      const response = await fetch('/api/users/hairdresser-accept-terms', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${session?.access_token}`
      ...
      setCurrentStep(4);
      navigate('/register/hairdresser/payment');
    } catch (error) {
  ...
  const handlePaymentSubmit = async (paymentData) => {
    try {
      ...
      const response = await fetch('/api/users/hairdresser-payment', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${session?.access_token}`
        ...
        body: JSON.stringify({
          ...
          paymentMethod: paymentData.method,
          paymentReference: paymentData.reference,
          amount: 1500 // 1,500 FCFA registration fee
        })
      });
      ...
      setCurrentStep(5);
      navigate('/register/hairdresser/success');
    } catch (error) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
      <div className="max-w-md w-full">
        <div className="text-center mb-6">
          ...
          <p className="text-gray-600 mt-2">Hairdresser Registration</p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-lg p-6">
          <Routes>
            ...
            <Route 
              path="/hairstyles" 
              element={<HairstyleSelection onSubmit={handleHairstyleSubmit} />} 
            />
            <Route 
              path="/terms" 
            ...
            <Route 
              path="/payment" 
              element={<PaymentForm onSubmit={handlePaymentSubmit} />} 
            />
            <Route 
              path="/success" 
